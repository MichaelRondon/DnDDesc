...
public class Test {
    ...
    public void testLevel() {
        try {
            Class<Level> declaringClass = ADnDClass.Level.FIRST
                    .getDeclaringClass();
            ...
            Method declaredMethod = declaringClass
                    .getDeclaredMethod("getRequiredXP");
            declaredMethod.setAccessible(true);
            Integer invoke = (Integer) declaredMethod
                    .invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(invoke.intValue(), 190000);
            declaredMethod = declaringClass
                    .getDeclaredMethod("getBaseSaveBonusGood");
            declaredMethod.setAccessible(true);
            invoke = (Integer) declaredMethod.invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(invoke.intValue(), 12);
            declaredMethod = declaringClass
                    .getDeclaredMethod("getBaseSaveBonusPoor");
            declaredMethod.setAccessible(true);
            invoke = (Integer) declaredMethod.invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(invoke.intValue(), 6);

            declaredMethod = declaringClass
                    .getDeclaredMethod("getBaseAttackBonusGood");
            declaredMethod.setAccessible(true);
            int[] invokeArray = (int[]) declaredMethod
                    .invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(
                    DnDUtil.getInstance().intArrayToString(invokeArray),
                    "20/15/10/5");
            declaredMethod = declaringClass
                    .getDeclaredMethod("getBaseAttackBonusAverage");
            declaredMethod.setAccessible(true);
            ...
            Assert.assertEquals(
                    ...
                    "15/10/5");
            declaredMethod = declaringClass
                    .getDeclaredMethod("getBaseAttackBonusPoor");
            declaredMethod.setAccessible(true);
            invokeArray = (int[]) declaredMethod.invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(
                    DnDUtil.getInstance().intArrayToString(invokeArray), "10/5");

            declaredMethod = declaringClass
                    .getDeclaredMethod("getMaxClassSkillRanks");
            declaredMethod.setAccessible(true);
            int i = (Integer) declaredMethod.invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(i, 23);
            declaredMethod = declaringClass
                    .getDeclaredMethod("getMaxNOClassSkillRanks");
            declaredMethod.setAccessible(true);
            @SuppressWarnings("unused")
            BigDecimal bigDecimal = (BigDecimal) declaredMethod
                    .invoke(ADnDClass.Level.TWENTY);
            Assert.assertEquals(new BigDecimal(11.5).doubleValue(), 11.5, 0.1);

            DnDCharacter testCharacter = this
                    .getTestCharacter(ADnDClass.Level.TWENTY);
    ...
    private DnDCharacter getTestCharacter() {
        ...
        try {
            declaredMethod = declaringClass
                    .getDeclaredMethod("getElfTestCharacter");
            declaredMethod.setAccessible(true);
    ...
    private DnDCharacter getTestCharacter(ADnDClass.Level level) {
        ...
        Field declaredField;
        try {
            declaredField = DnDCharacter.class
                    .getDeclaredField("descProperties");
            declaredField.setAccessible(true);
            @SuppressWarnings("unchecked")
            HashMap<String, Object> desc = (HashMap<String, Object>) declaredField
                    .get(testCharacter);
